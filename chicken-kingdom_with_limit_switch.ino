...
const float doorDisplacementHeight = 410.0; //in mm the height of open door, also the length of rope to get
...
// Limit switch
const boolean hasTopLimitSwitch=true;
#define GPIOTopLimitSwitch 1
...
String HTMLPage(){
  ...
  ptr +="</p>\n";
  if (hasTopLimitSwitch==true){
    if (digitalRead(GPIOTopLimitSwitch) == LOW){
      ptr += "<p> the top limit switch is detecting the door fully open</p>\n";      
    }
    else{
      ptr += "<p> the top limit switch is not detecting the door, it is not fully open</p>\n";      
    }    
  }
...
void setup()
{
  ...
  if (hasTopLimitSwitch == true) 
  {
    pinMode(GPIOTopLimitSwitch, INPUT_PULLUP);
    if (digitalRead(GPIOTopLimitSwitch) == LOW)
    {
      IsOpen=true;
    }
...
void openDoor()
{
  ...
  {
    int i=0;
    while (digitalRead(GPIOTopLimitSwitch) == HIGH && i<stepsToOpenOrClose*0.25*1.5){
      //here in the case of rotation to go up during 150% of calculated steps and still not reaching the top limit switch, we stop
      i=i+1;
      if (i%(int(1000/millsBetweenSteps))==0){
        server.handleClient();
      }
